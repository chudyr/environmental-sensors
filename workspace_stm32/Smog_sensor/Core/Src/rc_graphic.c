...
void rc_ssd1308_drawBitmap(int x, int y, const uint8_t *bitmap, uint8_t w, uint8_t h, uint8_t color)
{
    ...
    for(page=1; page<=bytehigh; page++)
    {
    	if(page==bytehigh && h%8 != 0) count_high=h%8;
    	else count_high=8;
    	for(j=0; j < w; j++)
    	{
    		temp_byte =	*(bitmap + j + (page-1)*w) ;
    		for(i=0; i<count_high ; i++)
    		{
    			if ( temp_byte >> i & 1  )
    			{
    				rc_ssd1309_setPixel(x+j, y+i+8*(page-1), color);
...
void rc_ssd1309_fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color) {
...
void writeLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1, uint16_t color) {
  ...
  for (; x0 <= x1; x0++) {
    if (steep) {
    	rc_ssd1309_setPixel(y0, x0, color);
    } else {
    	rc_ssd1309_setPixel(x0, y0, color);
    }
    err -= dy;
    if (err < 0) {
      y0 += ystep;
